...
class DenoiseDataSet(Dataset):
    def __init__(self, add_noise:str=None, crop_size:list=None, random_unshuffling_f=None, aug:list=None, n_repeat:int=1, n_data:int=None, ratio_data:float=None) -> None:
        '''
        ...
        self.random_unshuffling_f = random_unshuffling_f
    ...
    def _pre_processing(self, data):
        ...
        if self.random_unshuffling_f != None:
            data = self._random_unshuffling(self.random_unshuffling_f, data)

        return data

    def _random_unshuffling(self, f, data):
        no_f = np.random.randint(0, f**2)
        for key in data:
            if self._is_image_tensor(data[key]):
                tmp = data[key].unsqueeze(dim=1)
                data[key] = F.pixel_unshuffle(tmp, f)[:, no_f, ...]
        return data
